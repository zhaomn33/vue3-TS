  ...
  >
    ...
    >
      <component
        ...
        :class="{ 'modal-container__component-bottom-padding': !isShowCancelButton }"
      />
      <div
        v-if="!hideFooter"
        class="modal-container__footer"
        :class="{
          'container__footer-box': !showLeftFooter,
          'container__footer-show-right': showLeftFooter,
          'container__footer-show-button-border': !isShowCancelButton
        }"
      >
        <div
          v-if="showLeftFooter"
          class="modal-container-left__footer"
        >
...
export default defineComponent({
  ...
  setup(props, { attrs }: { attrs: any; }) {
    ...
    const handleDownLoad = async() => {
      const instance = refComponent.value
      try {
        await attrs.onDownLoad(instance, proxy)
        visible.value = false
      } catch (error) {
        if (process.env.NODE_ENV === 'development') {
          console.dir(error)
...
:deep() {
  ...
  .modal-container__component-bottom-padding {
    padding-bottom: 0;
  }
  ...
  .container__footer-box {
    display: flex;
    align-items: center;
    justify-content: end;
  }

  .container__footer-show-right {
    display: flex;
    align-items: center;
    justify-content: space-between;
  }

  .container__footer-show-button-border {
    border-top: 0;
  }
}
</style>
